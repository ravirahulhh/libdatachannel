...
void PrintStats() {
	...
	while (Running && !SendComplete) {
		...
		auto intervalMs = duration_cast<milliseconds>(now - lastTime).count();

		if (elapsed > 0 && Connected && intervalMs > 0) {
			// 计算间隔内的数据量
			uint64_t currentBytesSent = TotalBytesSent.load();
			uint64_t currentBytesAcked = TotalBytesAcked.load();
			uint64_t intervalBytesSent = currentBytesSent - lastBytesSent;
			uint64_t intervalBytesAcked = currentBytesAcked - lastBytesAcked;

			// 实时速度（基于间隔）
			double realtimeSendMbps = (intervalBytesSent * 8.0) / (intervalMs * 1000.0);
			double realtimeAckMbps = (intervalBytesAcked * 8.0) / (intervalMs * 1000.0);

			// 平均速度（从开始到现在）
			double avgMbps = (currentBytesAcked * 8.0) / (elapsed * 1000.0);

			double mbSent = currentBytesSent / (1024.0 * 1024.0);
			double mbAcked = currentBytesAcked / (1024.0 * 1024.0);
			double progress = (currentBytesAcked * 100.0) / (DataSizeGB * 1024 * 1024 * 1024);

			cout << "\r[Client] Queued: " << fixed << setprecision(2) << mbSent << " MB"
			     << " | Acked: " << mbAcked << " MB"
			     << " | Realtime: " << realtimeAckMbps << " Mbps"
			     << " | Avg: " << avgMbps << " Mbps"
			     << " | Pending: " << PendingBuffers << " | Progress: " << progress << "%" << endl;

			// 更新上次记录
			lastBytesSent = currentBytesSent;
			lastBytesAcked = currentBytesAcked;
			lastTime = now;
		}
	}
}
...
QUIC_STATUS QUIC_API ConnectionCallback(HQUIC Connection, void *Context,
	...
	case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT:
		cout << "\n[Client] Connection shutdown by transport: 0x" << hex
...
bool InitializeClient() {
	...
	const QUIC_REGISTRATION_CONFIG regConfig = {"speedtest-client",
	...
	status = MsQuic->ConfigurationOpen(Registration, &alpn, 1, &settings, sizeof(settings), nullptr,
	                                   &Configuration);
	if (QUIC_FAILED(status)) {
...
void SendData() {
	...
	// 创建流
	status = MsQuic->StreamOpen(Connection, QUIC_STREAM_OPEN_FLAG_NONE, StreamCallback, nullptr,
	                            &Stream);
	if (QUIC_FAILED(status)) {
	...
	while (sent < totalToSend && Running) {
		...
		QUIC_SEND_FLAGS flags =
		    (sent + chunkSize >= totalToSend) ? QUIC_SEND_FLAG_FIN : QUIC_SEND_FLAG_NONE;
